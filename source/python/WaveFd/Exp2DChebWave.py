...
class WaveField2D:
    ...
    def Next(self):
        ...
        x = np.arange(0.0, self.M*self.Ds, self.Ds)
        y = np.arange(0.0, self.N*self.Ds, self.Ds)
        ...
        for i in range(self.N):        
            Dx[i] = SplineDerivative(x, self.Pn[i]) # first derivative
            Dx[i] = SplineDerivative(x, Dx[i]/self.Rho[i]) # second with Rho
        ...
        for i in range(self.M):        
            Dy[i] = SplineDerivative(y, self.Pn[i]) # first derivative
            Dy[i] = SplineDerivative(y, Dy[i]/self.Rho[i]) # second with Rho
